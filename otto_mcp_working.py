#!/usr/bin/env python3
"""
OTTO - MCP-Enhanced System (Working Version)
============================================
...
class OttoMCPWorking:
    def __init__(self):
        ...
        self.mind_base = Path.home() / "Documents" / "Otto_MCP_Working_System"
        ...
        print(f"🧠 {self.name.upper()} - MCP-Enhanced System (Working)")
        print("=" * 70)
        ...
        print(f"🤖 OpenAI: {'Verbunden' if self.openai_client else 'Nicht verfügbar'}")
        print("=" * 70)
    ...
    def load_otto_self_narrative(self):
        ...
        if not self.otto_self_file.exists():
            otto_self = {
                ...
                "otto_core_identity": {
                    "name": "Otto",
                    "purpose": "Systemisch intelligenter Begleiter",
                    "core_traits": [
                ...
                "self_memories": [
                    {
                        "date": datetime.now().strftime("%Y-%m-%d"),
                        "memory": "Ich bin Otto, erschaffen von Ben im MCP-System",
                        "importance": "hoch",
    ...
    def generate_intelligent_response(self, text: str, mcp_analysis: dict, mcp_level: int):
        ...
        mcp_response = mcp_response(mcp_analysis)
        ...
        if self.openai_client:
            try:
                enhanced_response = self.enhance_with_openai(text, mcp_response, mcp_analysis, mcp_level)
    ...
    def enhance_with_openai(self, text: str, mcp_response: str, mcp_analysis: dict, mcp_level: int):
        """Erweitert Antwort mit OpenAI und MCP-Level"""
        try:
            ...
            response = openai.ChatCompletion.create(
                model="gpt-3.5-turbo",
                messages=[
                    {"role": "system", "content": f"Du bist Otto, ein systemisch intelligenter Begleiter mit MCP-Level {mcp_level}."},
